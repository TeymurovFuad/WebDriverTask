...
namespace WebDriverTask.Pages.Gmail.Folders
{
    public class Sent : MainPage, IMailFolder
    {
        ...
        {
            WaitUntilElementIsInteractable(Core.WebDriver.Driver.GetDriver().FindElement(By.XPath(PathToMails)));
            List<IWebElement> sentMails = WebDriver.GetDriver().FindElements(By.XPath(PathToMails)).ToList();
            return sentMails;
        }
